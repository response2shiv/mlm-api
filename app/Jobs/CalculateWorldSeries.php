...
class CalculateWorldSeries implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($startDate, $endDate, $truncate)
    {
    ...
    {
        //
        Log::info('Running worldseries calculation...', [get_called_class(), 'startDate' => $this->startDate, 'endDate' => $this->endDate, 'truncate' => $this->truncate]);
        
        Log::info('worldseries Calculation done');
        ...
        $orders = Order::getWorldSeriesOrdersByUser($this->startDate, $this->endDate);
        
        $progress = 1;
        $this->setProgressMax(count($orders));
        ...
        foreach($orders as $order){
            ...
            foreach($orderItems as $orderItem){
                ...
                if($orderItem->prod_id > 0 && $orderItem->prod_id <5){
                    $type = 'enrollment';
                    $valid = true;
                }
            }
        ...
        # Calculate the Bonus Runs for the currently month
        $bonus_runs = $this->calculateBonusRuns($this->startDate, $this->endDate);
    ...
    public function calculateBonusRuns($startDate, $endDate) 
    {
        $month = \Carbon\Carbon::createFromFormat('Y-m-d', $startDate)->month;
  
        $results = WorldSeriesBonusRunsService::calculate($startDate, $endDate);
