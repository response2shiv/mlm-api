...
class NetworkMerchants
{
    ...
    public function setLogin($paymentMethodType)
    {
        ...
        if ($paymentMethodType == PaymentMethodType::TYPE_T1_PAYMENTS ||  $paymentMethodType == PaymentMethodType::TYPE_T1_PAYMENTS_SECONDARY_CC) {
            $this->login['username'] = Config::get('api_endpoints.t1Username');
            $this->login['password'] = Config::get('api_endpoints.t1Password');
        } else if ($paymentMethodType == PaymentMethodType::TYPE_PAYARC) {
        ...
        } else if ($paymentMethodType == PaymentMethodType::TYPE_METROPOLITAN) {
    ...
    // Transaction public Functions

    public function doSale($amount, $ccnumber, $ccexp, $cvv = "", $currency = null)
    {
    ...
    public function _doPost($query)
    {
        ...
        curl_setopt($ch, CURLOPT_PROXY, Config::get('api_endpoints.vgs_proxy'));
        curl_setopt($ch, CURLOPT_PROXYPORT, Config::get('api_endpoints.vgs_port'));
        curl_setopt($ch, CURLOPT_PROXYUSERPWD, Config::get('api_endpoints.vgs_userpaswd'));
